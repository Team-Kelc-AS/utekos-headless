...
import { analyzeBoundary, type GateId } from './analyzePdpBoundary'
...
test('everything imported by a \'use client\' module is client code', () => {
  const report = analyze({
...
test('a \'use server\' module is not pulled into the client bundle', () => {
  const report = analyze({
    'entry.tsx': `import { Island } from './Island'
export default function Page() { return <Island /> }`,
    'Island.tsx': `'use client'
...
export async function save() { return heavy() }`,
    'heavy.ts': 'export function heavy() { return \'x\'.repeat(1000) }'
  })
...
export const options = () => queryOptions({
  ...
  queryFn: async () => fetchThing()
    ...
    'fetchThing.ts': 'export async function fetchThing() { return 1 }'
  })
...
export function Island(props: Props) { return props.model.id }`,
    'model.ts': 'export type ProductPurchaseModel = { id: string }'
  })
