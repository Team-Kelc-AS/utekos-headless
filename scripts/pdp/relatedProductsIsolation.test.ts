...
test('related products are excluded from the critical PDP data path', async () => {
  const [
...
test('related products render behind a dedicated lower-page Suspense boundary', async () => {
  const [productPageViewSource, asyncRelatedProductsSource] =
    await Promise.all([
      readSource(
  ...
  assert.match(productPageViewSource, /AsyncRelatedProducts/)
  ...
  assert.match(asyncRelatedProductsSource, /try \{/)

  assert.match(asyncRelatedProductsSource, /return null/)
  ...
  assert.match(asyncRelatedProductsSource, /<RelatedProducts/)

  const accordionIndex = productPageViewSource.indexOf(
    '<ProductPageAccordion'
  )

  const recommendationsIndex = productPageViewSource.indexOf(
...
test('related products query Shopify recommendations by current product handle', async () => {
  const [querySource, cachedRelatedSource] = await Promise.all([
    readSource(
...
test('product catalog webhooks expire related-products Runtime Cache tags', async () => {
