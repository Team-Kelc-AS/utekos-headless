...
function hasDirective(file: SourceFile, directive: string): boolean {
...
export function analyzeBoundary(
  ...
  while (queue.length > 0) {
    ...
    const role: ModuleRole =
      item.role === 'client' || declaresClient ? 'client' : 'server'
    ...
    for (const imported of getValueImports(file)) {
      ...
      if (role === 'server' && hasDirective(target, CLIENT_DIRECTIVE)) {
  ...
  for (const boundaryFile of boundaryModules) {
    ...
    collectFullProductPropViolations(
      project.getSourceFileOrThrow(path.join(root, boundaryFile)),
      boundaryFile,
      gateViolations
    )
  }
  ...
  return {
    ...
    gateViolations: dedupeViolations(gateViolations).sort((a, b) =>
      a.file === b.file ?
        a.line - b.line
...
function collectServerActionQueryFnViolations(
  ...
  for (const declaration of file.getImportDeclarations()) {
    ...
    if (!target || !hasDirective(target, SERVER_ACTION_DIRECTIVE)) {
      continue
    }
