 ...
 */
function summarizeProductRequests(requests, origin) {
  ...
  return {
    authoritativeProductRefetchCount: serverActions.length,
...
async function measureHardReload({
  ...
  return withPage(browser, cpuThrottle, async page => {
    ...
    return {
      ...
      productRequests: summarizeProductRequests(
        requests,
        baseUrl
      ),
      lab: {
        ...
        scriptEvaluationMs: perf.scriptEvaluationMs,
...
async function measureClientNavigation({
  ...
  return withPage(browser, cpuThrottle, async page => {
    ...
    return {
      ...
      // from the RSC responses in productRequests.
      scripts: summarizeScripts(requests),
      productRequests: summarizeProductRequests(
        requests,
        baseUrl
...
function aggregate(report) {
  ...
  return [...groups.entries()].map(([key, items]) => {
    ...
    return {
      ...
      authoritativeProductRefetchCount: pick(
        item =>
          item.productRequests?.authoritativeProductRefetchCount
      ),
      rscFetchBytes: pick(
        item => item.productRequests?.rscFetchBytes
      ),
      ...
      scriptEvaluationMs: pick(
        item => item.lab?.scriptEvaluationMs
      ),
  ...
  for (const row of summary) {
    console.log(
      [
        ...
        row.authoritativeProductRefetchCount,
        row.lcpMs ?? '-',
        row.cls ?? '-',
