...
const COMPLETE_HTML = `<!DOCTYPE html><html lang="no"><head><title>ok</title></head><body><script>self.__next_f.push([1,"0:null\\n"])</script></body></html>`
const TRUNCATED_HTML = `<!DOCTYPE html><html lang="no"><head><title>bad</title></head><body><script>self.__next_f.push([1,"0:`
const COMPLETE_FLIGHT = `1:I[100,["/_next/static/chunks/a.js","/_next/static/chunks/b.js"],"Module"]\n0:["$","html",null,{}]\n2:null\n`
const TRUNCATED_FLIGHT = `1:I[100,["/_next/static/chunks/a.js"\n0:["$","html"`
const EMPTY_FLIGHT = ''
...
test('flight completeness fails on truncated or empty payloads', () => {
  ...
  assert.ok(truncated.reasons.includes('missing_trailing_newline'))
...
test('buildRequest emits RSC header and _rsc query for flight', () => {
  ...
  assert.equal(buildRequest('/', 'html').headers.accept.includes('text/html'), true)
})
...
test('evaluateResponse fails truncated html even with HTTP 200', () => {
  ...
  const evaluated = evaluateResponse(response, TRUNCATED_HTML, 'html')
...
test('evaluateResponse passes complete flight payloads', () => {
  ...
  const evaluated = evaluateResponse(response, COMPLETE_FLIGHT, 'flight')
...
test('toSafeArtifact never embeds full bodies', () => {
  ...
  assert.ok(artifact.bodyHead.length <= 120)
  assert.ok(artifact.bodyTail.length <= 120)
  assert.equal(artifact.headers['set-cookie'], '[redacted]')
  assert.equal(Object.hasOwn(artifact, 'body'), false)
})

test('runPprRscProbe compares cold and warm and fails on truncation', async () => {
  ...
  const fetchImpl = async url => {
    ...
    // Force truncation on warm HTML for /
    if (key.endsWith('/') && n > 1) {
  ...
  assert.ok(report.coldWarmComparison.length >= 4)
  const homeHtml = report.coldWarmComparison.find(
    row => row.route === '/' && row.kind === 'html'
  )
  assert.equal(homeHtml?.cold?.ok, true)
  assert.equal(homeHtml?.warm?.ok, false)
  assert.equal(homeHtml?.warm?.vercelCache, 'HIT')
  assert.throws(() => assertProbePassed(report), /PPR\/RSC probe failed/)
})

test('runPprRscProbe passes when every cold/warm response is complete', async () => {
  const fetchImpl = async url => {
    const isFlight = String(url).includes('_rsc=')
    return new Response(isFlight ? COMPLETE_FLIGHT : COMPLETE_HTML, {
      status: 200,
      headers: {
        'content-type': isFlight ? 'text/x-component' : 'text/html; charset=utf-8',
        'x-vercel-cache': 'HIT'
      }
    })
  }
  ...
  assert.equal(
    report.coldWarmComparison.every(row => row.bothComplete),
    true
  )
