...
export function pickHeaders(headers, names) {
  ...
  const get =
    typeof headers.get === 'function'
      ...
      : name => {
          const record = /** @type {Record<string, string | null | undefined>} */ (
            headers
          )
          return (
            record[name] ??
  ...
  for (const name of names) {
    ...
    out[name.toLowerCase()] = value == null || value === '' ? null : String(value)
  }
  return out
}
...
export function redactHeaders(headers) {
  ...
  const entries =
    typeof headers.entries === 'function'
      ? [...headers.entries()]
      : Object.entries(
          /** @type {Record<string, string | null | undefined>} */ (headers)
        )
 ...
 * @param {number} [edge]
 */
export function bodyEdges(body, edge = 120) {
  if (body.length <= edge * 2) {
    return { head: body, tail: body }
  }
  return {
    head: body.slice(0, edge),
    tail: body.slice(-edge)
  }
}
...
export function analyzeHtmlCompleteness(body) {
  ...
  if (!hasDoctype && !hasHtmlOpen) reasons.push('missing_html_document_start')
  if (!hasHtmlClose) reasons.push('missing_html_close')
  if (!hasBodyClose) reasons.push('missing_body_close')
  if (!endsWithHtmlClose) reasons.push('does_not_end_with_html_close')
  if (nextFPushes === 0) reasons.push('missing_next_f_flight_bootstrap')
  if (looksTruncatedMarker) reasons.push('truncated_stream_marker')
...
export function analyzeFlightCompleteness(body, options = {}) {
  ...
  const flightRows = nonEmptyLines.filter(line => FLIGHT_ROW_RE.test(line))
  ...
  const contentTypeOk = /text\/x-component|text\/plain|application\/text/i.test(
    contentType
  )

  if (body.length === 0) reasons.push('empty_body')
  if (contentType && !contentTypeOk) reasons.push('unexpected_content_type')
  if (flightRows.length === 0) reasons.push('missing_flight_rows')
...
export function analyzeCompleteness(kind, body, options = {}) {
  ...
  if (kind === 'flight') return analyzeFlightCompleteness(body, options)
  return {
    kind,
    complete: false,
...
export function toSafeArtifact(input) {
  ...
  return {
    ...
    cachePhase: input.cachePhase ?? null,
