...
Overordnet mål er økt merkevarekjennskap knyttet til merkevaren Utekos. 
Nettbutikken utekos.no er avhengig av innsikt i brukeratferd, og ikke minst er algoritimene på annonseplattformene vi benytte avhengig av å få tilsendt optimal data.
Hvis denne linken er suboptimal eller i verste fall ryker, så har bedriften ingen fremtid. Dette skal priotireres på høyeste nivå. 

Videre ønsker vi å gjøre det tydelig hvorfor data sendes til Supabase,
...
gjeldende trackingkontrakten. Web-GTM v135 var live ved aktivering og er nå
avløst av v141; v133 introduserte mappingen fra isolert
...
| Meta Pixel / CAPI             | Meta-attribusjon og budoptimalisering                  | Kanoniske Pixel/CAPI-events, purchase og samtykkede IDs                                 | Event Match Quality, Dataset Quality, ads learning                                | Automatisk/inferred eventtaksonomi eller skriverett uten godkjenning | Web-GTM v141, app-eid Pixel-bro og 17 Meta CAPI-adaptere er live. Identisk navn/UUID er wire-verifisert for `InteractWithAccordion`; Meta svarte `events_received=1`, og bro/GTM deler duplicate-suppression. |
...
| 4. Provider dispatch | Kvalifiserte events forsøkes levert til Meta, Google og Microsoft med retry og sannferdig statusklassifisering         | retry-dispatch, provider adapters, [provider-finality-runbook](docs/analytics/provider-finality-runbook.md) | Lokal attemptstatus + separat providerlevering + separat attribusjon/dedupe; ingen aksept-til-finalitet-inferens |
...
| Provider health       | KRI-22 snapshot `2026-07-31T01:04:19Z` hadde 0 `pending`, `processing`, `retry_scheduled` og `failed`. KRI-23 read-only trace `01:51:42Z` observerte tre Google-rader som `accepted_unverified` / `provider_processing`; dette er forventet providerbehandling, ikke lokal `processing`. En annen Google-rad hadde terminal `SUCCESS`; Meta `events_received=1` og Microsoft HTTP 200 forble `accepted_unverified`. Siste 24 timer i KRI-22 hadde Meta 598 accepted-samples med p95 2 826 ms og Google ett sample på 4 021 ms. |
...
| 4. Provider dispatch | Meta/Google er aktive; Microsoft UET CAPI er aktiv for `add_to_cart`, `begin_checkout` og `purchase`. Nye attempts vekkes målrettet med eksakt ID; femminutterscronen er recovery/fallback.                                                                                                                                                    | `accepted_unverified` beviser bare forventet adapterkvittering. Kun Google har autoritativ per-request reconciliation. Meta/Microsoft har ingen radfinalitet; attribusjon og dedupe er egne providerobservasjoner. Nyere Microsoft-rader uten `msclkid` er korrekt `skipped_unqualified`. | Operatører må skille Queue-redelivery fra database-retry, attemptstatus fra providerlevering og providerlevering fra attribusjon/dedupe. | Følg [provider-finality-runbook](docs/analytics/provider-finality-runbook.md), behold exact-attempt-kontrakten og opprett separat schemaoppfølging før de tre aksene eventuelt lagres uavhengig. |
