...
Før-måling for server-first-refaktoreringen av produktsiden
(KRI-5). Ingen produksjonskode er endret i denne saken.
...
Måleharnesset er sjekket inn og er også etter-måleverktøyet for
KRI-11. Metrikkdefinisjonene må ikke endres mellom kjøringene.
...
- **Autoritative produktrefetcher** kan ikke identifiseres på
  URL: PDP-en bruker en Server Action som TanStack `queryFn`, og
  de går som POST til side-URL-en med `next-action`-header.
  Harnesset klassifiserer på header.
...
| Metrikk                          | techdown-plain | techdown-variant | comfyrobe     |
| -------------------------------- | -------------- | ---------------- | ------------- |
| RSC-payload for ruten            | 11,4 KiB       | 13,4 KiB         | 13,3 KiB      |
| Nettverkskall                    | 51             | 51               | 51            |
| **Rutespesifikk JS overført**    | **258,0 KiB**  | **258,0 KiB**    | **258,0 KiB** |
| **Rutespesifikk JS ukomprimert** | **845,2 KiB**  | **845,2 KiB**    | **845,2 KiB** |
| Autoritative produktrefetcher    | 0              | 0                | 0             |
| CLS                              | 0,0028         | 0,0028           | 0,0028        |
| TBT etter transisjon             | 328 ms         | 315 ms           | 334 ms        |
| Long-task-tid                    | 678 ms         | 687 ms           | 684 ms        |
...
### 1. Produktrefetch etter mount skjer ikke i praksis — men mekanismen står der

Issuet antar autoritativ produktrefetch i browseren. **Målt: 0
refetcher**, også etter 130 s med `visibilitychange`- og
`focus`-sykluser.
...
over Server Action. Målet «0 autoritative produktrefetcher» er
altså allerede oppfylt ved uhell, ikke ved design, og budsjettet
må formuleres som _0 muligheter_, ikke bare _0 observerte_.
...
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Storefront API-kall per hard reload / klientnavigasjon                  | **Blokkert.** Krever serverlogger eller instrumentert Preview. Ingen Shopify-credentials i dette miljøet, og produksjon eksponerer ikke kallantall. Statisk verifisert i stedet: 1× `getProduct` (dedupet), 1× `getProducts(first: 24)` for relaterte, 1× `getProductOptions` (`no-store`) per request. |
...
| ----------------------------------------------------------------- | ----------------------------- | -------------------------------------------------- |
| Autoritative produktrefetcher i browser etter mount               | 0 observert, mekanisme finnes | 0 — og `getProductAction` skal ikke være `queryFn` |
...
1. **KRI-12** (Cache Components/PPR) først — `await connection()`
   i `page.tsx` gjør PDP fullt dynamisk og påvirker alle
   payload-tall. Refaktorerer vi over et ustabilt cachefundament,
   blir før/etter ikke sammenlignbart.
...
6. **KRI-11** — etter-måling med samme harness, samme flagg,
   samme miljø.
