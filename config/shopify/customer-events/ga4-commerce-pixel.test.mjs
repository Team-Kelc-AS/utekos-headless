...
function checkoutEvent(overrides = {}) {
  return {
    ...
    context: {
      document: {
        location: { href: 'https://kasse.utekos.no/checkouts/abc/thank-you' }
      }
    },
    data: {
      checkout: {
...
test('fails closed until Shopify grants analytics consent', () => {
  ...
  assert.equal(commands(harness)[0][2].analytics_storage, 'granted')
...
test('sends purchase with the canonical Data Manager transaction id', () => {
  ...
  const purchaseCommands = commands(harness).filter(
    command => command[0] === 'event' && command[1] === 'purchase'
  )
...
test('matches the server purchase transaction, value, currency, and items', () => {
  ...
  assert.equal(browserPurchase.value, serverPurchase.conversionValue)
  assert.deepEqual(
...
test('rejects checkout purchases without a Shopify order legacy id', () => {
  ...
  assert.equal(
    commands(harness).filter(command => command[1] === 'purchase').length,
    0
  )
})
...
test('maps payment_info_submitted to add_payment_info', () => {
  ...
  assert.equal(paymentCommands[0][2].items[0].item_id, '48249962135800')
})
...
test('revoking analytics consent updates the Google tag and blocks events', () => {
  ...
  assert.equal(
    commands(harness).filter(command => command[0] === 'event').length,
    0
  )
})
