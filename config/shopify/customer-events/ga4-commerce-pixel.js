(function () {
  ...
  function checkoutItems(checkout) {
    ...
    return lineItems.reduce(function (items, lineItem) {
      ...
      var itemId =
        numericShopifyId(variant && variant.id, 'ProductVariant') ||
        numericShopifyId(lineItem && lineItem.id, 'CheckoutLineItem')
      var itemName =
        lineItem && typeof lineItem.title === 'string' ?
          lineItem.title.trim()
        : ''

      if (!itemId || !itemName || !Number.isInteger(quantity) || quantity <= 0) {
        return items
      }
      ...
      var price =
        lineValue !== undefined ? lineValue / quantity : variantPrice
      var item = {
        item_id: itemId,
        item_name: itemName,
        quantity: quantity
  ...
  function sendPaymentInfo(event) {
    ...
    if (!parameters.currency || parameters.items.length === 0) return
  ...
  function sendPurchase(event) {
    ...
    var shipping = finiteAmount(
      checkout && checkout.shippingLine && checkout.shippingLine.price
    )
