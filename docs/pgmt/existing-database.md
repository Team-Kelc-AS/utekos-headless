...
This is one of two on-ramps into pgmt: `pgmt init` **adopts an existing database** (the schema is already there — pgmt records a baseline and starts managing changes from that point), while `pgmt migrate provision` **stands up a fresh database** from that baseline. This guide covers the first; see [Provisioning a New Environment](https://pgmt.dev/docs/guides/baseline-management#provisioning-a-new-environment) for the second.

## Import Workflow

[Section titled “Import Workflow”](https://pgmt.dev/docs/guides/existing-database/#import-workflow)

Point pgmt at your existing database:

```bash
mkdir my-app && cd my-apppgmt init --dev-url postgres://localhost/your_existing_db
```
...
[Section titled “Shadow Database for Extension-Heavy Schemas”](https://pgmt.dev/docs/guides/existing-database/#shadow-database-for-extension-heavy-schemas)
...
The same image must be available wherever pgmt runs (your machine, CI, teammates), since `migrate apply` and validation all spin up a shadow. See [Configuration](https://pgmt.dev/docs/reference/configuration) for all shadow options.

During import, pgmt also checks what the fresh shadow already provides: schemas like PostGIS’s `tiger` and `topology` exist before your schema is applied, so they belong to the image, not your project. pgmt offers to exclude them from management (all pre-selected) and records the choice in `pgmt.yaml` under `objects.exclude.schemas` — without this, every diff would fight the image over objects it recreates.

## Fix Dependencies

[Section titled “Fix Dependencies”](https://pgmt.dev/docs/guides/existing-database/#fix-dependencies)

Validation might fail with errors like:

```
❌ Schema validation failed
ERROR: relation "users" does not existCONTEXT: function create_user_profile()
```

This means a function references a table that pgmt couldn’t detect as a dependency. Add it explicitly:

```
-- schema/functions.sql-- require: tables.sql
...
[Section titled “Team Onboarding”](https://pgmt.dev/docs/guides/existing-database/#team-onboarding)

When teammates clone the repo, they provision a fresh database from the baseline:

```
git clone your-repo && cd your-repocreatedb myapp_devpgmt migrate provision --target-url postgres://localhost/myapp_dev
```
...
[Section titled “CLI Options”](https://pgmt.dev/docs/guides/existing-database/#cli-options)

```
# Interactive (recommended)pgmt init --dev-url postgres://localhost/your_db
# Non-interactive with baselinepgmt init --dev-url postgres://localhost/your_db --create-baseline
# Skip baseline (rare - only for empty databases)pgmt init --dev-url postgres://localhost/your_db --no-baseline
```

## Migrating from Other Tools

[Section titled “Migrating from Other Tools”](https://pgmt.dev/docs/guides/existing-database/#migrating-from-other-tools)
...
[Section titled “Reorganizing Schema Files”](https://pgmt.dev/docs/guides/existing-database/#reorganizing-schema-files)

pgmt generates schema files grouped by object type (`tables.sql`, `views.sql`). You may want to reorganize by domain after import. See [Schema Organization](https://pgmt.dev/docs/guides/schema-organization) for patterns.

## Troubleshooting

[Section titled “Troubleshooting”](https://pgmt.dev/docs/guides/existing-database/#troubleshooting)

**Hidden dependencies in functions:** Functions that query tables have dependencies pgmt can’t detect automatically. Add `-- require:` statements.

**Extension-owned objects appearing:** pgmt filters these automatically. If you see PostGIS types or similar, check your extension configuration.

**Circular dependencies:** Reorganize into separate files or merge the circular objects into one file.

[](https://pgmt.dev/docs/guides/modules)