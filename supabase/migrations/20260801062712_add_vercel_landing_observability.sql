...
comment on column ops.vercel_edge_request_observations.vercel_log_id is
  'Vercel log id and immutable ingestion idempotency key.';
comment on column ops.vercel_edge_request_observations.edge_request_id is
  'Optional correlation UUID parsed only from the strict landing-edge structured message.';
comment on column ops.vercel_edge_request_observations.fbclid_hmac is
  'HMAC-SHA256 digest when fbclid is present, computed with a dedicated required receiver secret; the raw fbclid is never stored.';
...
comment on column ops.landing_consent_observations.edge_request_id is
  'Primary correlation id without a foreign key because Vercel Log Drain delivery is asynchronous.';
...
begin
  ...
  where observation.observed_at < v_now - interval '30 days'
    and not ops.has_active_privacy_retention_exception(
      ...
      observation.edge_request_id::text,
      v_now
    );
...
comment on function ops.purge_expired_landing_observations() is
  'Deletes Vercel edge-request, trace and landing-consent observations after 30 days unless a time-limited privacy retention exception is active.';
...
do $schedule_landing_observation_purge$
declare
...
begin
  if to_regclass('cron.job') is null then
    raise exception using
      errcode = '55000',
      message = 'pg_cron cron.job is required for landing observation retention';
  end if;
...
$schedule_landing_observation_purge$;
