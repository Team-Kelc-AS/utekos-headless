...
function RatingItem({
  ...
  const partialPointWidth =
    isPartialPoint && shouldShowFilled ?
      `${partialPoint * 100}%`
    : undefined
  ...
  const getRatingPoint = React.useCallback(
    (event: React.MouseEvent<HTMLLabelElement>) => {
      ...
      if (width === 0 || precision <= 0 || precision > 1)
        return 0
  ...
  return (
    <>
      <Comp
        ...
        htmlFor={
          readOnly ? undefined : `${ratingIconId}-${point}`
        }
...
function Rating({
  ...
  return (
    <div
      ...
      aria-label={
        readOnly ? `${value} of ${max} stars` : 'Rating'
      }
      {...props}
    >
