...
export async function fetchProductCardsWithRetry(input: {
  ...
  const budgetMs =
    input.budgetMs ?? RELATED_PRODUCTS_TOTAL_BUDGET_MS
  ...
  try {
    ...
    try {
      ...
      const jitterMs = getRelatedProductsRetryJitterMs(
        input.random
      )
      ...
      if (
        ...
        remainingMs <=
          jitterMs + RELATED_PRODUCTS_MIN_RETRY_BUDGET_MS
      ) {
        throw error
      }
      ...
      if (
        retryTimeoutMs < RELATED_PRODUCTS_MIN_RETRY_BUDGET_MS
      ) {
        throw error
      }
