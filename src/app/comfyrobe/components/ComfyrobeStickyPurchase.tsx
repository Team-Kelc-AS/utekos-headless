...
export function ComfyrobeStickyPurchase({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (reportedPageViewId.current === pageView.pageViewId)
        return
        ...
        >
          <div className='dark:border-dark-foreground/15 dark:bg-dark-background/96 flex items-center gap-2 rounded-full border border-foreground/15 bg-background/96 p-2 text-foreground shadow-[0_14px_45px_rgba(0,0,0,0.3)] backdrop-blur-md'>
            ...
            <BrandBadge
              ...
              className='font-google-sans min-h-11 shrink-0 px-4 py-2 text-sm font-bold transition-[filter,transform] hover:brightness-105 active:scale-[0.985]'
            >
              <button
                type='button'
                onClick={handlePurchase}
