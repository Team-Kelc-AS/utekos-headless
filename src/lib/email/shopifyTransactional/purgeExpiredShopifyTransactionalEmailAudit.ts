...
export async function purgeExpiredShopifyTransactionalEmailAudit(
  now = new Date()
): Promise<number> {
  if (!Number.isFinite(now.getTime())) {
    throw new Error(
      'shopify_transactional_email_audit_purge_input_invalid'
    )
  }
  ...
  if (error || !Number.isInteger(data) || data < 0) {
    throw new Error(
      'shopify_transactional_email_audit_purge_failed'
    )
  }
