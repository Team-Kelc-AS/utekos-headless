...
test('publishes every public Shopify variant with matching Meta content IDs', () => {
  ...
  assert.equal(rows[1]?.price, '1790.00 NOK')
...
test('uses the newest included Shopify variant for Last-Modified', () => {
  ...
  assert.equal(document.offerCount, 2)
  assert.equal(
    document.lastModified,
    'Sun, 23 Aug 2026 10:00:00 GMT'
  )
})
...
test('fails closed when no active public variants remain', () => {
  assert.throws(
    ...
    /contains no active public offers/
  )
})
