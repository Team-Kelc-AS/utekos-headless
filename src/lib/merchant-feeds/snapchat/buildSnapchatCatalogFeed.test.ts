import assert from 'node:assert/strict'
import test from 'node:test'

import type { CatalogSyncProduct } from '@/lib/catalog-sync/types'

import {
  buildSnapchatCatalogFeed,
  buildSnapchatCatalogFeedDocument,
  SNAPCHAT_CATALOG_FEED_COLUMNS
} from './buildSnapchatCatalogFeed'

const product: CatalogSyncProduct = {
  id: 'gid://shopify/Product/100',
  title: 'Shopify title that must not be used',
  handle: 'utekos-techdown',
  productType: 'Uteklær',
  descriptionHtml: '<p>Shopify description that must not be used.</p>',
  vendor: 'Utekos',
  status: 'ACTIVE',
  updatedAt: '2026-08-23T08:00:00Z',
  featuredImage: {
    url: 'https://cdn.shopify.com/s/files/techdown.jpg?v=1'
  },
  images: [],
  variants: {
    edges: [
      {
        node: {
          id: 'gid://shopify/ProductVariant/200',
          title: 'Havdyp / Middels',
          sku: 'UTEKOS-HAV-MIDDELS',
          barcode: '4006381333931',
          price: '1790',
          compareAtPrice: '1990',
          inventoryQuantity: 4,
          availableForSale: true,
          updatedAt: '2026-08-23T09:00:00Z',
          image: {
            url: 'https://cdn.shopify.com/s/files/variant,medium.png?v=2'
          },
          selectedOptions: [
            { name: 'Farge', value: 'Havdyp' },
            { name: 'Størrelse', value: 'Medium' },
            { name: 'Kjønn', value: 'Unisex' }
          ],
          weight: null,
          weightUnit: 'kg',
          customLabel0: null,
          customLabel1: null,
          customLabel2: null,
          customLabel3: null,
          customLabel4: null
        }
      },
      {
        node: {
          id: 'gid://shopify/ProductVariant/201',
          title: 'Havdyp / Stor',
          sku: 'UTEKOS-HAV-STOR',
          barcode: null,
          price: '1790',
          compareAtPrice: null,
          inventoryQuantity: 0,
          availableForSale: false,
          updatedAt: '2026-08-23T10:00:00Z',
          image: null,
          selectedOptions: [
            { name: 'Farge', value: 'Havdyp' },
            { name: 'Størrelse', value: 'Large' },
            { name: 'Kjønn', value: 'Unisex' }
          ],
          weight: null,
          weightUnit: 'kg',
          customLabel0: null,
          customLabel1: null,
          customLabel2: null,
          customLabel3: null,
          customLabel4: null
        }
      }
    ]
  }
}

function parseFeedRows(feed: string) {
  const lines = feed.trimEnd().split('\r\n')
  const columns = lines[0]?.split('\t') ?? []

  return lines.slice(1).map(line => {
    const values = line.split('\t')

    return Object.fromEntries(
      columns.map((column, index) => [
        column,
        values[index] ?? ''
      ])
    )
  })
}

test('builds an in-stock Snapchat variant feed from curated Utekos presentation data', () => {
  const feed = buildSnapchatCatalogFeed([product])
  const rows = parseFeedRows(feed)

  assert.equal(
    feed.split('\r\n')[0],
    SNAPCHAT_CATALOG_FEED_COLUMNS.join('\t')
  )
  assert.equal(rows.length, 1)
  assert.equal(rows[0]?.id, '200')
  assert.equal(rows[0]?.item_group_id, '100')
  assert.equal(
    rows[0]?.title,
    'Utekos TechDown™ Havdyp - Middels'
  )
  assert.doesNotMatch(rows[0]?.title ?? '', /\//)
  assert.equal(rows[0]?.availability, 'in stock')
  assert.equal(rows[0]?.price, '1990.00 NOK')
  assert.equal(rows[0]?.sale_price, '1790.00 NOK')
  assert.equal(rows[0]?.brand, 'Utekos')
  assert.equal(rows[0]?.gtin, '4006381333931')
  assert.equal(rows[0]?.color, 'Havdyp')
  assert.equal(rows[0]?.size, 'Middels')
  assert.equal(rows[0]?.gender, 'unisex')
  assert.equal(rows[0]?.age_group, 'adult')
  assert.equal(rows[0]?.size_system, 'EU')
  assert.equal(rows[0]?.size_type, 'regular')
  assert.equal(rows[0]?.adult, 'no')
  assert.equal(rows[0]?.condition, 'new')
  assert.equal(rows[0]?.google_product_category, '203')
  assert.equal(
    rows[0]?.product_type,
    'Apparel & Accessories > Clothing > Outerwear'
  )
  assert.equal(
    rows[0]?.image_link,
    'https://cdn.shopify.com/s/files/variant%2Cmedium.png?v=2'
  )
  assert.equal(
    rows[0]?.link,
    'https://utekos.no/produkter/utekos-techdown?farge=havdyp&storrelse=middels&kjonn=unisex&utm_source=snapchat&utm_medium=paid_social&utm_campaign=snapchat_catalog&utm_content=200'
  )
  assert.equal(
    SNAPCHAT_CATALOG_FEED_COLUMNS.includes(
      'additional_image_link' as never
    ),
    false
  )
  assert.ok(feed.endsWith('\r\n'))
})

test('uses only included offer timestamps for Last-Modified', () => {
  const document = buildSnapchatCatalogFeedDocument([product])

  assert.equal(
    document.lastModified,
    'Sun, 23 Aug 2026 09:00:00 GMT'
  )
})

test('fails closed when no active in-stock variants remain', () => {
  assert.throws(
    () =>
      buildSnapchatCatalogFeed([
        {
          ...product,
          variants: {
            edges: [product.variants.edges[1]!]
          }
        }
      ]),
    /contains no active in-stock offers/
  )
})
