...
STATUS 2026-08-19: DEN ORDINÆRE UTVIKLINGS- OG PRODUKSJONSFLYTEN ER
ENDRET TIL ÉN KANONISK MAIN-ONLY-RUTE. DEN PRIMÆRE LOKALE CHECKOUTEN,
GITHUB `MAIN` OG AKTIV VERCEL-PRODUKSJON SKAL ALLTID PEKE PÅ SAMME
COMMIT NÅR EN OPPGAVE ELLER RELEASE AVSLUTTES. ARBEID SKJER DIREKTE PÅ
`MAIN`; WORKTREES OG FEATURE-BRANCHER ER IKKE NORMALFLYT OG KREVER ET
EKSPLISITT, NAVNGITT MIGRASJONS- ELLER HENDELSESUNNTAK MED OBLIGATORISK
REKONSILIERING OG OPPRYDDING. `PNPM SYNC "<COMMIT MESSAGE>"` ER ENESTE
ORDINÆRE COMMIT-, PUSH- OG DEPLOYKOMMANDO. HELE STATUSEN OG DIFFEN SKAL
DERFOR VÆRE ÉN INTENDERT RELEASE FØR KOMMANDOEN KJØRES, OG
`MAIN...ORIGIN/MAIN` SKAL VÆRE `0 0` BÅDE UMIDDELBART FØR OG ETTER.
`GITHUB.AUTOALIAS=FALSE` ER FJERNET SLIK AT PUSH TIL VERCELS KONFIGURERTE
PRODUKSJONSBRANCH IGJEN KAN OPPDATERE PRODUKSJONSDOMENENE AUTOMATISK.
SE `AGENTS.md` OG DEN AUTORITATIVE RELEASEMODELLEN I `DEPLOYMENT.md`.
DENNE STATUSEN AVLØSER NORMALFLYTEN FRA 2026-08-10; HISTORISKE PR-,
PREVIEW- OG MANUELLE PROMOTERINGSBEVIS BEHOLDES SOM HISTORIKK, IKKE SOM
GJELDENDE STANDARD.

STATUS 2026-08-18: PINTEREST-REPARASJONENS CHECKOUT-OPPFØLGING ER
PRODUKSJONSDEPLOYERT VIA PR `#199`, RUNTIME-COMMIT
`988F5BCE7E3674C0813DEC5D23C1B0E27670819E` OG MERGE-SHA
`EDF1CB545BDB0F06C1482B99CB1D0BB3FA7A6F64`. EN PRODUKTKONTEKST UTEN
KUNDE- ELLER PERSONDATA FØLGER NÅ SHOPIFY-KURVEN TIL ORDREN OG
GJENOPPRETTER EKSAKT VARIANT-ID, MERKE OG KATEGORI I `PURCHASE` FOR
ORDINÆR SHOPIFY CHECKOUT OG KLARNA EXPRESS. SHOPIFY ADMIN-
REKONSILIERING BRUKER I TILLEGG DE OFFISIELLE `VENDOR`- OG
`PRODUCTTYPE`-FELTENE. ATTRIBUTTVERDIEN ER FAIL-CLOSED, BEGRENSET TIL
PROSJEKTETS VALIDERTE SHOPIFY-GRENSE OG FABRIKKERER ALDRI PRODUKT- ELLER
KUNDEDATA. 16 MÅLRETTEDE TESTER, LINT, APP- OG EDGE-TYPESCRIPT,
DIFF-KONTROLL, CODEQL OG METICULOUS MED NULL FORSKJELLER PÅ SEKS
SKJERMBILDER ER GRØNNE. PREVIEW
`DPL_9HF2JEOWTFKCWKEUFE1B9SVWPDRE` VAR `READY`, SVARTE 200 OG LASTET
RIKTIG GTM-CONTAINER. GIT-INTEGRASJONENS EKSAKTE `MAIN`-ARTEFAKT
`DPL_8Y6XHRCUG5DTJKKWNIQA5CJHJYKF` VAR `READY` UTEN
PRODUKSJONSALIAS. DET BLE KONTROLLERT PROMOTERT TIL
PRODUKSJONSDEPLOYMENT `DPL_5XOUKPBMU2ZVSR42EFAQJCRE6LJB`, SOM ER
`READY`, HAR EKSAKT MERGE-SHA OG EIER `UTEKOS.NO`, `WWW.UTEKOS.NO` OG
`FEED.UTEKOS.NO`. ALLE TRE DOMENER, GTM/SGTM OG VERCEL-RUNTIMELOGGEN ER
GRØNNE. INGEN SYNTETISK PINTEREST-HENDELSE, SHOPIFY-ORDRE,
GTM-PUBLISERING, SUPABASE-MIGRASJON, ENV-ENDRING ELLER PROVIDERMUTASJON
BLE UTFØRT. FØRSTE NATURLIGE KVALIFISERTE CHECKOUT/PURCHASE MÅ FORTSATT
BEKREFTE DET NYE PRODUKTFELTET I PINTERESTS PROVIDERRESPONS; FORRIGE
PRODUKSJONSDEPLOYMENT `DPL_9SRPK6U5HC7B3UITYOMWWUNSOS2A` ER
ROLLBACK-KANDIDAT.

STATUS 2026-08-18: PINTEREST TAG + CONVERSIONS API-HENDELSESKVALITET ER
REPARERT OG PRODUKSJONSDEPLOYERT VIA PR `#197`, RUNTIME-COMMIT
`24B202436A175FD78B6376BE8B84834799642F13` OG MERGE-SHA
`BC91BEBB07C0B54C2731198F7CEE7DBF877F0EBB`. REPARASJONEN BEVARER
`EPIK` GJENNOM DEN SAMTYKKEBASERTE SHOPIFY CHECKOUT-ATTRIBUSJONEN,
SENDER HASHET `EXTERNAL_ID` TIL TAG ENHANCED MATCH, SENDER HASHET BY,
LAND, GYLDIG TOBOKSTAVS REGION OG POSTNUMMER TIL CAPI, OG VIDEREFØRER
EKSAKT SHOPIFY-VARIANT-ID, PRODUKTMERKE OG PRODUKTKATEGORI. E-POST OG
NAVN FABRIKKERES IKKE FØR KUNDEN FAKTISK HAR OPPGITT DEM; `CLICK_ID`
SENDES NÅR EN REELL PINTEREST-KLIKKØKT HAR `_EPIK`. MÅLRETTEDE TESTER
PASSERTE 28/28, TYPESCRIPT, EDGE-TYPES, LINT, NEXT TYPEGEN,
PRODUKSJONSBYGG OG GTM/SGTM-SMOKE ER GRØNNE. GIT-PREVIEW
`DPL_7X35S2ZKJLUBPJM2WBG28VSKQUIC` VAR `READY` FOR RUNTIME-COMMITEN.
ETTER MERGE PRODUSERTE GIT-INTEGRASJONEN DET EKSAKTE `MAIN`-ARTEFAKTET
`DPL_DNEFS7OEBYMHCFMEUYDRRYVLCNT` SOM `READY`, MEN UTEN
PRODUKSJONSALIAS FORDI REPOSITORYETS EKSISTERENDE
`GITHUB.AUTOALIAS=FALSE` ER BEHOLDT. DEN EKSPLISITT GODKJENTE
PRODUKSJONSRELEASEN BLE DERFOR FULLFØRT SOM KONTROLLERT PROMOTERING AV
DET SAMME GIT-ARTEFAKTET, IKKE SOM ET NYTT `VERCEL --PROD`-BYGG.
PRODUKSJONSDEPLOYMENT `DPL_9SRPK6U5HC7B3UITYOMWWUNSOS2A` ER `READY`,
HAR EKSAKT MERGE-SHA OG EIER `UTEKOS.NO`, `WWW.UTEKOS.NO` OG
`FEED.UTEKOS.NO`. OFFENTLIG HTML/JS, SAMTYKKEGATE OG GTM/SGTM ER
VERIFISERT; VERCEL HAR NULL ERROR-/FATALLOGGER. ÉN NATURLIG
`VIEW_ITEM` ETTER CUTOVER HADDE EXTERNAL ID, BY/LAND/REGION/POSTNUMMER,
PRODUKT-ID, MERKE OG KATEGORI, OG PINTEREST RETURNERTE `PROCESSED` MED
1/1 MOTTATT OG BEHANDLET. HENDELSEN HADDE IKKE `CLICK_ID`, SOM ER
FORVENTET UTEN EN REELL PINTEREST-BETALT KLIKKØKT. INGEN SYNTETISK
HENDELSE, GTM-PUBLISERING, SUPABASE-MIGRASJON, ENV-ENDRING ELLER ANNEN
PROVIDERMUTASJON BLE UTFØRT. PINTEREST-RAPPORTEN FRA 17. AUGUST BRUKER
ET 14-DAGERS VINDU OG MÅ DERFOR FØLGES OPP ETTER NY KVALIFISERT TRAFIKK;
PROVIDERENS AKSEPT ER IKKE DET SAMME SOM OPPDATERT MATCHSCORE ELLER
ATTRIBUSJON. FORRIGE PRODUKSJONSDEPLOYMENT
`DPL_GCJKHTAX3P5URUZXPBBZ8BVCLJKD` ER ROLLBACK-KANDIDAT.

STATUS 2026-08-13: BOTID/KASADA ER FJERNET FRA DEN LOKALE
RELEASEKANDIDATENS TIDSKRITISKE TELEMETRIBANE FOR `/API/EVENTS/*`,
`/API/OBSERVABILITY/LANDING-CONSENT` OG
`/API/OBSERVABILITY/PAGE-VIEW-DISPATCH`. NETTLESEREN PATCHER IKKE LENGER
`FETCH`/XHR FOR DISSE KALLENE, SERVEREN KJØRER IKKE LENGER SYNKRON
`CHECKBOTID`, OG NEXT-WRAPPER, RUNTIMEAVHENGIGHET OG KASADA-PATCH ER
FJERNET. SIGNERT SYNTETISK TRAFIKK, SAME-ORIGIN, SIGNERT KORRELASJON,
BODY-/SCHEMAVALIDERING, SAMTYKKE, IDEMPOTENS OG VERCEL FIREWALL ER
BEHOLDT. TO GRENSEREGRESJONER, 32 PAGEVIEW-/OBSERVASJONSTESTER, ÅTTE
EDGE-/PROXYTESTER, LINT, DIFF-KONTROLL, NEXT TYPEGEN, TYPESCRIPT,
MCP-DIAGNOSTIKK OG PRODUKSJONSBYGG MED 136/136 SIDER ER GRØNNE. LOKALE
META-IN-APP-LIGNENDE VALIDERINGSKALL SVARTE PÅ 4–15 MS UTEN BOTID-
VENTING; PRODUKSJONENS GTM/SGTM-BASELINE ER OGSÅ GRØNN. DEN BREDE
ANALYTICS-SERVERPAKKEN HAR 429/438 BESTÅTT; DE NI FEILENE ER EKSISTERENDE
TESTHARNESS-/SENTRY- OG UTDATERT CRON-FORVENTNING UTENFOR DENNE DIFFEN.
RUNTIME-COMMIT `EC73A91E0B5F2B142BD9D11C86D6751724CC952E` ER DIREKTE
PUSHET TIL `MAIN` SOM AUTORISERT NØDRELEASE UTEN PREVIEW. GIT-UTLØST
VERCEL-DEPLOYMENT `DPL_9FU7RRMPBNTWDCGADZTALP2RJWA8` ER `READY` OG EIER
`UTEKOS.NO`, `WWW.UTEKOS.NO` OG `FEED.UTEKOS.NO`. FULL META-ANDROID-
IN-APP-LASTING HADDE 113 REQUESTS, `LANDING-CONSENT` 202 PÅ 154 MS OG
NULL BOTID/KASADA/KPSDK-RESSURSER. TO NATURLIGE PAGEVIEWS MED `FBP`,
`FBC` OG `EXTERNAL_ID` BLE AKSEPTERT MED 202; META RETURNERTE
`EVENTS_RECEIVED=1`, TRACE-ID OG NULL MELDINGER FOR BEGGE PÅ 292/314 MS.
GTM/SGTM ER 200/NO-STORE/CACHE MISS, OG VERCEL HAR INGEN RUNTIMEFEIL.
FORRIGE PRODUKSJONSDEPLOYMENT
`DPL_3O1YHXCJCCBAYKGC82L4ESMY8EJU` ER ROLLBACK-KANDIDAT.

STATUS 2026-08-12: TECHDOWN-KJØPSOPPLEVELSEN PÅ `/SKREDDERSY-VARMEN`
ER PRODUKSJONSDEPLOYERT VIA PR `#157` FRA RUNTIME-COMMIT
`5E6AB717E00D09F5E86C349C7F907BD14C244496` OG SQUASH-MERGE
`2CFFA07C84632D5A480E7A39A226E399E1D89263`. GIT-UTLØST VERCEL-
DEPLOYMENT `DPL_EVBCHCIKHXXTTAKXAVXEL4WLDDYR` ER `READY` OG EIER
`UTEKOS.NO`, `WWW.UTEKOS.NO` OG `FEED.UTEKOS.NO`. LOKAL NEXT TYPEGEN,
TYPESCRIPT, PRODUKSJONSBYGG, PRETTIER OG DIFF-KONTROLL ER GRØNNE.
PREVIEW `DPL_DXNGGY3DVCF5S4S1KZ1QSGG2CT1D` VAR `READY` FOR EKSAKT
RUNTIME-COMMIT, MEN DOM-SMOKE VAR BLOKKERT AV VERCEL DEPLOYMENT
PROTECTION OG ER IKKE RAPPORTERT SOM BESTÅTT. OFFENTLIG MOBILSMOKE
VERIFISERTE SIDETITTEL, FAQ, ÉN KLARNA-PLASSERING MED `NB-NO`,
STØRRELSESRADIOER, MOBILFANER OG STØRRELSESANBEFALING. EN REELL
EKSTRA-STOR TECHDOWN-VARIANT BLE LAGT I HANDLEKURVEN, VIST MED KORREKT
DELSUM, FULGT TIL SHOPIFY CHECKOUT UTEN BETALING ELLER ORDRE OG DERETTER
FJERNET IGJEN. NETTLESERKONSOLL OG VERCEL ERROR-/FATAL-LOGGER VAR TOMME.
INGEN SUPABASE-, ENV-, GTM-, SHOPIFY-KATALOG- ELLER ANNEN
PROVIDERMUTASJON INNGÅR.

STATUS 2026-08-11: MICROSOFT MERCHANT-FEEDENS AKTIVE SORTIMENTSKONTRAKT
ER AVGRENSSET TIL UTEKOS MIKROFIBER FJELLBLÅ, COMFYROBE OG UTEKOS
TECHDOWN. ALLE UTEKOS DUN-, STAPPER- OG BUFF-TILBUD SAMT MIKROFIBER
VARGNATT ER EKSKLUDERT FØR TSV-RADEN BYGGES. FOR KATALOGTILSTANDEN VED
RELEASE ER FORVENTET OUTPUT NI TILBUD: TO MIKROFIBER FJELLBLÅ, TRE
COMFYROBE OG FIRE TECHDOWN. ALLE VARIANTTITLER BRUKER FORMATET
`[PRODUKTNAVN] [FARGE] – [STØRRELSE]`; KJØNN SENDES KUN I EGET FELT.
SAMME TITTELFORMAT ER KOBLET TIL GOOGLE MERCHANT API SIN
`PRODUCTINPUT`-BYGGER. META-KATALOG `690208780604782` BRUKER DERIMOT
BATCH API/PARTNER-INTEGRASJON UTEN EN REPOSITORY-EID FEED ELLER
KATALOGSYNK; META KAN DERFOR IKKE ENDRES VARIG GJENNOM DENNE
RELEASEN. MICROSOFT-OPPDATERING OG PROVIDERENS
PROSESSERTE KATALOGSTATUS GJENSTÅR ETTER APPDEPLOY. INGEN SUPABASE-,
ENV-, GTM-, SENTRY-, SHOPIFY- ELLER ANNEN PROVIDERMUTASJON INNGÅR.

STATUS 2026-08-10 (HISTORISK, AVLØST 2026-08-19): PRODUKSJONSFLYTEN VAR
NORMALISERT TIL REN BRANCH FRA
FERSK `ORIGIN/MAIN`, GIT-UTLØST VERCEL PREVIEW, REVIEWET PULL REQUEST OG
DERETTER GIT-UTLØST PRODUKSJONSDEPLOY FRA DEN EKSAKTE MERGE-COMMITEN PÅ
`MAIN`. DEN TIDLIGERE `PNPM RUN SYNC`-SNARVEIEN ER FJERNET FORDI DEN KJØRTE
`GIT ADD .` OG KUNNE TA MED URELATERTE ENDRINGER. DIREKTE `VERCEL --PROD`,
DEPLOY HOOK OG MANUELL ALIASPROMOTERING ER BREAK-GLASS, IKKE NORMAL FLYT.
WEB-GTM V141 ER DIREKTE LEST TILBAKE SOM PUBLISERT; V140 ER UMIDDELBAR
ROLLBACK. HISTORISKE V135-/V136-STATUSLINJER ER MERKET SOM AVLØST.
...
MAPPERTEST ER LAGT TIL. WEB-GTM V135 VAR VERIFISERT DIREKTE FOR DENNE
HISTORISKE RELEASEN; DEN ER AVLØST AV V141 FRA 2026-08-10. FØRSTE PLANLAGTE
...
STATUS 2026-07-17 (HISTORISK): DEN ENKLE GIT-FLYTEN STAGET ALLE
ENDRINGER MED `GIT ADD .`, COMMITTET OG PUSHET. DEN BLE AVLØST
2026-08-10, MEN EN MAIN-ONLY-VARIANT MED STRENGERE SYNKRONISERINGS-,
DIFF-, DOKUMENTASJONS- OG PRODUKSJONSBEVIS ER GJENINNFØRT 2026-08-19.
SE NYESTE STATUS OG `DEPLOYMENT.md` FOR GJELDENDE KONTRAKT.
