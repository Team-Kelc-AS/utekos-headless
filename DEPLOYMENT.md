...
makes retries idempotent per dataset and event, so a successful primary run
makes the second schedule a no-op while a transient first-run failure retains
one same-day recovery opportunity. The token is sent in the authorization
header and is never included in the request URL or response.
...
| Pinterest Tag + CAPI env | Set `NEXT_PUBLIC_PINTEREST_TAG_ID`, `PINTEREST_CONVERSIONS_API_ENABLED=true`, `PINTEREST_CONVERSIONS_ACCESS_TOKEN` (events/conversion scope, not `catalogs:read`), and `PINTEREST_AD_ACCOUNT_ID` in Vercel Production. Do not add a GTM Pinterest tag. | Vercel after code merge; redeploy after env change. | HTML `#pinterest-tag-canonical-browser` with `data-tag-id`; consented production `pintrk` + `s.pinimg.com/ct/core.js`; CAPI `ops.provider_dispatch_attempts` `provider='pinterest'` with the same `event_id`; skip `missing_capi_token` or `insufficient_pinterest_user_identity` when unqualified. |
...
npm run mcp:build
npm run mcp:doctor
# Historical run also used the aggregate mcp:commerce-tracking:doctor.
# That script is no longer registered; use the active target-specific
# doctors and tracking smoke tests for the changed surface instead.
